...
public class Task_AngularDev : ATask
{
    ...
    {
        float goalAngle = targetLocation.rotation.x - oldPose.rootMotionInfo.value.rotation.x; //pseudocode. difference in rads of rotation between character and goal. wont work because oldPose is not in same coordinate space. unsure if rotX is correct val to check.

        float candidateAngle = newPose.rootMotionInfo.value.rotation.x - oldPose.rootMotionInfo.value.rotation.x; //psuedocode. difference in rads of rotation between character and candidate. unsure if rotX is correct val to check.
